...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_VOLTAGE, DP_TYPE_VALUE},
	{DPID_SALT, DP_TYPE_BOOL},
  {DPID_BITTER, DP_TYPE_BOOL},
};
...
void all_data_update(void)
{
  ...
  // This code is automatically generated for the platform. Please modify each function according to the actual data
    ...
    mcu_dp_value_update(DPID_VOLTAGE,voltage); //VALUE型数据上报;
		mcu_dp_bool_update(DPID_SALT,salt_set); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_BITTER,bitter_set); //BOOL型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_salt_handle
功能描述 : 针对DPID_SALT的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_salt_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    salt = mcu_get_dp_download_bool(value,length);
		salt_set = salt;
    if(salt == 0) {
        //开关关
				salt_set = 0;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_SALT,salt);
		ret = mcu_dp_bool_update(DPID_BITTER,bitter_set);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_bitter_handle
功能描述 : 针对DPID_BITTER的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_bitter_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char bitter;
    
    bitter = mcu_get_dp_download_bool(value,length);
		bitter_set = bitter;
    if(bitter == 0) {
        //开关关
				bitter_set = 0;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_BITTER,bitter);
		ret = mcu_dp_bool_update(DPID_SALT,salt_set);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
  ...
  ***********************************/
  ...
  switch(dpid)
  {
        ...
        break;
				case DPID_SALT:
            //咸味处理函数
            ret = dp_download_salt_handle(value,length);
        break;
        case DPID_BITTER:
            //苦味处理函数
            ret = dp_download_bitter_handle(value,length);
        break;
